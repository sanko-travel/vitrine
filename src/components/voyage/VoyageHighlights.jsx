...
export default function VoyageHighlights({ data }) {
  ...
  const getMaxTranslate = () => {
    ...
    const lastCardCenter =
      padLeft + (count - 1) * (CARD_WIDTH + GAP) + CARD_WIDTH / 2;
  ...
  const scrollToCard = (index) => {
    ...
    const targetTranslate = Math.max(
      0,
      Math.min(maxTranslate, cardCenter - wrapper.clientWidth / 2),
    );
          ...
          >
            {data.highlights.map((item, i) => (
              ...
              >
                ...
                <span
                  className={`absolute top-3 right-3 z-10 px-4 py-1.5 rounded-full font-accent font-semibold text-sm ${["bg-coral text-white", "bg-teal text-white", "bg-yellow text-white", "bg-coral text-white"][i % 4]}`}
                >
                  {item.title}
                </span>
                ...
                <div className="absolute bottom-0 left-0 right-0 translate-y-[calc(100%-3rem)] group-hover:translate-y-0 transition-transform duration-500 ease-out">
                  ...
                  <div className="flex justify-center pb-1 group-hover:opacity-0 transition-opacity duration-300">
                    <svg
                      className="w-5 h-5 text-white drop-shadow-md animate-bounce"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={2.5}
                      stroke="currentColor"
